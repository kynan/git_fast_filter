...
def _timedelta_to_seconds(delta):
    ...
    """
    offset = delta.days * 86400 + delta.seconds + (delta.microseconds +
                                                   0.0) / 1000000
    return round(offset)
...
class FastExportFilter(object):
    ...
    def _parse_user(self, usertype):
        ...
        # Translate when into a datetime object, with corresponding timezone
        # info
    ...
    def _parse_commit(self):
        ...
        if self._currentline.startswith('author'):
            (author_name, author_email,
             author_date) = self._parse_user('author')
    ...
    def run(self, *args):
        ...
        if len(args) > 0:
            if type(args[0]) == str:
                # If repo-name provided, set up fast_export process pipe as
                # input
        ...
        # If we created fast_import process, close pipe and wait for it to
        # finish
        if need_wait:
            self._output.close()
            output_pipe.wait()
